  ...
  (let ((name (get-connection-line conn)))
    (when name
	  ...
	  (progn
	    ...
	    (show-current-turn name)
	    (show-top-card name)
	    (format (socket-stream conn) "This is a text-based game. Enter \"help\" for help on the commands.~c~%" #\Return)
	    (force-output (socket-stream conn)))))
    name))
	...
	(cond
	  ((equal command "help")
	   (let ((stream (socket-stream conn)))
	     (format stream "say <text>:~cSay something to the other players.~c~%" #\Tab #\Return)
	     (format stream "players:~cPrint the list of players along with the number of cards in their hand.~c~%" #\Tab #\Return)
	     (format stream "hand:~cShow your hand.~c~%" #\Tab #\Return)
	     (format stream "top:~cShow the top card.~c~%" #\Tab #\Return)
	     (format stream "turn:~cShow whose turn it is.~c~%" #\Tab #\Return)
	     (format stream "play <card number> <color?>:~cPlay the card identified by number from your hand. If it is a wild card, the color argument is required.~c~%" #\Tab #\Return)
	     (format stream "draw:~cDraw a card. If you can play the card, you have to play it after drawing it.~c~%" #\Tab #\Return)
	     (format stream "uno:~cCall uno if you have a single card.~c~%" #\Tab #\Return)
	     (force-output stream)))
