	...
	(cond
	  ((equal command "help")
	   (let ((stream (socket-stream conn)))
	     (format stream "say <text>: Say something to the other players.~c~%" #\Return)
	     (format stream "players: Print the list of players along with the number of cards in their hand.~c~%" #\Return)
	     (format stream "hand: Show your hand.~c~%" #\Return)
	     (format stream "top: Show the top card.~c~%" #\Return)
	     (format stream "turn: Show whose turn it is.~c~%" #\Return)
	     (format stream "play <card number> <color?>: Play the card identified by number from your hand. If it is a wild card, the color argument is required.~c~%" #\Return)
	     (format stream "draw: Draw a card. If you can play the card, you have to play it after drawing it.~c~%" #\Return)
	     (format stream "uno: Call uno if you have a single card.~c~%" #\Return)
	     (force-output stream)))
